...
var app = express();
var remotePlugins = [];
ROOT_DROMEDARY_FILE = require(_getDromedaryFile());
 ...
 * @param dromedaryFile dromedary file
 * @returns {string} string blank space separated string with names of all remote plugins
 * @private
 */
function _getRemotePlugins(dromedaryFile) {
    Object.keys(dromedaryFile.routes).forEach(function (route) {
        Object.keys(dromedaryFile.routes[route].flow).forEach(function (stepName) {
            var plugin = dromedaryFile.routes[route].flow[stepName];
        ...
        Object.keys(df.routes[route]['flow']).forEach(function (stepName) {
            ...
            if (pluginName === "composite") {
                ...
                if(remotePlugins.indexOf(pluginName) == -1){
                    plugin = require(ROOT_DROMEDARY_FILE['env']['localPlugins'] + "/" + pluginName)
                } else {
...
function start() {
    ...
    _getRemotePlugins(ROOT_DROMEDARY_FILE);
